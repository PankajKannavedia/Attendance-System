...
export const approveLeave = async (req: Request, res: Response) => {
  try {
    ...
    await Leave.findOneAndUpdate({ applicationId: id as string }, { status });
...
// Early Leave Methods
export const applyEarlyLeave = async (req: Request, res: Response) => {
  try {
...
export const getPersonalLeaveApplications = async (req: Request, res: Response) => {
  try {
    const { employeeId } = req.params;
    const leaves = await Leave.find({ employeeId: employeeId as string });
    res.json(leaves);
  } catch (error) {
...
export const getEarlyLeaveApplications = async (req: Request, res: Response) => {
  try {
...
export const getPersonalEarlyLeaveApplications = async (req: Request, res: Response) => {
  try {
    const { employeeId } = req.params;
    const leaves = await EarlyLeave.find({ employeeId: employeeId as string });
    res.json(leaves);
  } catch (error) {
...
export const approveEarlyLeave = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { status } = req.body;
    await EarlyLeave.findOneAndUpdate({ applicationId: id as string }, { status });
