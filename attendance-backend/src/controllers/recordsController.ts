...
export const getAllEmployeeRecords = async (req: Request, res: Response) => {
  try {
    const today = new Date().toISOString().split('T')[0]!;
    const records = await Attendance.find({ date: today } as any).sort({ createdAt: -1 });
    const overtimes = await Overtime.find({ otDate: today } as any);
    
    // Map to match GAS output
    const formattedRecords = records.map(r => {
      let action = "N/A";
      if (r.punchOut?.time) action = "Out";
      else if (r.punchIn?.time) action = "In";
...
export const getMonthlyRecords = async (req: Request, res: Response) => {
  try {
...
export const get30DaySummary = async (req: Request, res: Response) => {
  try {
    ...
    // Group by employeeId
    const employeeData: Record<string, { employeeId: string, name: string, attendance: Record<string, string>, presentCount: number, absentCount: number }> = {};
